...
select
    ...
    GROUP_CONCAT(creator.creator order by creator.idx  SEPARATOR '||') as `creator`,
    GROUP_CONCAT(creator_author.creator order by creator_author.idx  SEPARATOR '||') as `creator_author`,
    GROUP_CONCAT(creator_editor.creator order by creator_editor.idx  SEPARATOR '||') as `creator_editor`,
    GROUP_CONCAT(creator_organization.creator order by creator_organization.idx  SEPARATOR '||') as `creator_organization`,
    GROUP_CONCAT(idno.idno order by idno.idx  SEPARATOR '||') as `idno`,
    GROUP_CONCAT(idno_cobiss.idno order by idno_cobiss.idx  SEPARATOR '||') as `idno_cobiss`,
    GROUP_CONCAT(idno_issn.idno order by idno_issn.idx  SEPARATOR '||') as `idno_issn`,
    GROUP_CONCAT(idno_sistory.idno order by idno_sistory.idx  SEPARATOR '||') as `idno_sistory`,
    GROUP_CONCAT(`year`.`year` order by `year`.idx  SEPARATOR '||') as `year`,
    GROUP_CONCAT(title.title order by title.idx  SEPARATOR '||') as `title`,
    GROUP_CONCAT(publisher.publisher order by publisher.idx  SEPARATOR '||') as `publisher`,
    GROUP_CONCAT(place.place order by place.idx  SEPARATOR '||') as `place`,
    GROUP_CONCAT(addidno.addidno order by addidno.idx  SEPARATOR '||') as `addidno`,
    GROUP_CONCAT(addtitle.addtitle order by addtitle.idx  SEPARATOR '||') as `addtitle`,
    GROUP_CONCAT(volume.volume order by volume.idx  SEPARATOR '||') as `volume`,
    GROUP_CONCAT(issue.issue order by issue.idx  SEPARATOR '||') as `issue`,
    GROUP_CONCAT(`page`.`page` order by `page`.idx  SEPARATOR '||') as `page`,
    GROUP_CONCAT(edition.edition order by edition.idx  SEPARATOR '||') as `edition`,
    GROUP_CONCAT(`source`.`source` order by `source`.idx  SEPARATOR '||') as `source`,
    GROUP_CONCAT(`online`.`online` order by `online`.idx  SEPARATOR '||') as `online`,
    GROUP_CONCAT(strng.strng order by strng.idx  SEPARATOR '||') as `strng`,
    GROUP_CONCAT(note.note order by note.idx  SEPARATOR '||') as `note`,
    GROUP_CONCAT(project_link.proj_id order by project_link.link_id  SEPARATOR '||') as `proj_id`
