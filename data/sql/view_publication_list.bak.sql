CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_publication_list`
AS
  select
    ...
    (
      ...
      order by `publication_creator`.`idx`) AS `creator_organization`,(select group_concat(`publication_idno`.`idno` separator '||')
    ...
    (
      ...
      order by `publication_idno`.`idx`) AS `idno_cobiss`,(select group_concat(`publication_idno`.`idno` separator '||')
      from (`publication_idno` join `codes_pub_idno` on((`codes_pub_idno`.`code_id` = `publication_idno`.`code_id`)))
    ...
    (
      ...
      where (`publication_year`.`pub_id` = `publication`.`pub_id`)
